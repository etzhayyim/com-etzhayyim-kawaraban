(ns kawaraban.cacao
  "Agent-side CACAO issuance (JVM). kawaraban mints its OWN server-verifiable CACAO to
  authenticate a publish to the aozora PDS (com.atproto.repo.createRecord) — no human-handed
  token. 1:1 port of `tashikame.cacao` (ADR-2607110200; keep in sync) — see that namespace's
  docstring for the full design rationale (per-actor key model, depth-1 self-mint, SIWE/CBOR
  wire format ported from `kotoba.cacao`).

  The private key is persisted to `.kawaraban/identity.edn` (gitignored) — NEVER commit a
  private key."
...
;; ───────── pure CACAO builders (mirror of kotoba.cacao / tashikame.cacao) ─────────
...
(defn canonical-graph
  "The deterministic graph CID for one of kawaraban's databases."
  [did db-name]
  (graph-cid-from-name (str "kotobase/db/" did "/" db-name)))

(def default-db-name
  "kawaraban's primary database — the news-medium article/mention/wire graph."
  "kawaraban")
...
(defn load-or-create-identity!
  "Per-actor key: load kawaraban's persisted Ed25519 identity at `path`, or generate +
  persist one on first run (only the b64 key material is stored)."
  [path]
